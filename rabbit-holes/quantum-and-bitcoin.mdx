...
About **30.5 percent of circulating supply** sits at quantum-exposed addresses today - close to one bitcoin in three. That figure carries a correction worth stating outright. The original Baseline 2026 Q2 measurement counted 5,071,264 BTC, or 25.3 percent. A later parser pass (methodology v1.1) folded in roughly another 1.04 million BTC of nested-segwit reuse the first walk had skipped, which brings the strict figure to about 30.5 percent.

The breakdown below is that v1.0 baseline - 5,071,264 BTC across 12,749,047 addresses, before the nested-segwit coins were folded in. Some are dormant. Most are not. The largest single bucket is not Satoshi - it is **1,896,840 BTC at modern segwit (P2WPKH) addresses that got reused at least once**. Real users, real wallets, real habits.

Snapshot: 2026-06-07, block 952,694, methodology v1.0. The full breakdown:
...
<figure>
  ...
  <figcaption>Six buckets, ordered by BTC at risk. Always-exposed types (P2PK, bare multisig, P2TR) in deep orange. Reuse-exposed types in lighter orange. This is the v1.0 Baseline 2026 Q2 breakdown (5,071,264 BTC); the v1.1 correction adds roughly 1.04M BTC of nested-segwit reuse on top. Snapshot 2026-06-07, base block 952,694. Source: ChainQuery.</figcaption>
</figure>
...
BIP-361 cites "over 34 percent of all BTC" as quantum-exposed. ChainQuery's strict number is about 30.5 percent. The gap is methodology.
...
A third independent measurement anchors the upper bound empirically. The [Wicked Smart Bitcoin quantum exposure dashboard](https://wickedsmartbitcoin.com/quantum_exposure) applies the conservative BIP-361-style counting rule directly to chain data and reports 34.55 percent at block 951,000 - matching BIP-361's analytic figure almost exactly. Methodology v1.1 closed most of the original gap: folding in the roughly 1.04 million BTC of nested-segwit reuse (P2SH-wrapped P2WPKH and P2WSH, whose pubkey is revealed in the witness) narrowed the distance to Wicked's number to about 4 points. The residual is small, and it is not explained by puzzle scripts - timelock-only, hash-preimage, and OP_RETURN-shaped redeems together hold only tens of BTC. Three measurements, transparent gap.

Either way: roughly a third of the supply, today.
