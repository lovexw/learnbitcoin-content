...
sources:
  ...
  - { label: "Bitcoin Wiki - Privacy (wallet fingerprinting)", url: "https://en.bitcoin.it/wiki/Privacy#Wallet_fingerprinting" }
  ...
  - { label: "CoinDesk - Roman Storm guilty of unlicensed money transmitting conspiracy in partial verdict (2025)", url: "https://www.coindesk.com/policy/2025/08/06/roman-storm-guilty-of-unlicensed-money-transmitting-conspiracy-in-partial-verdict" }
  - { label: "The Block - Roman Storm retrial pushed back to April 2027 (2026)", url: "https://www.theblock.co/news/regulation/2026-08-26-tornado-cash-roman-storm-retrial-april-2027-412761" }
...
> **Pro tip:** Your wallet's defaults do a lot of this work for the analyst. Pay a `bc1p` Taproot address from a wallet that sends change to a `bc1q` address and the change is simply the output that matches your inputs. Bitcoin Core has matched the change type to the destination type when it can since version 23.0 in 2022, and the better wallets do the same. Before worrying about which CoinJoin coordinator is still running, check what your own wallet does with change. Credit to u/Kind-Economics-7184 on r/Bitcoin for the framing.
...
[PayJoin](/glossary/payjoin) is a normal payment with one twist: the receiver adds an input of their own before the sender signs. The transaction now has inputs from two wallets, which makes it a lie to the common-input heuristic, and on its face it looks like an ordinary payment. That second property is the point. Unlike a CoinJoin, a PayJoin is not identifiable as a privacy transaction, and every one that exists makes the common-input rule slightly less reliable for every other transaction on the chain. The specification is [BIP-78](https://github.com/bitcoin/bips/blob/master/bip-0078.mediawiki), from 2019, and a second version, BIP-77, removes the requirement that the receiver be online at the moment of payment. Adoption is still small. Both sides need wallet support, and most merchants have never heard of it.

> **Pro tip:** "Looks like an ordinary payment" is only true at the level of inputs and outputs. Wallet software leaves fingerprints, in nSequence values, signature and script choices, fee and coin-selection habits, and how the two parties behave before and after, and an analyst who can tell that two different wallet engines built one transaction can often recover which inputs were whose. The arms race has moved from "which inputs belong together" to "which wallet produced this," and PayJoin only stays ahead while wallets keep their fingerprints small and similar. Credit to Antidote47k on BitcoinTalk for pressing this point.
