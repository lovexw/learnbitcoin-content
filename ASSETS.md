...
| **Send. Settle. Done.** | `bitcoin-lifecycle.mp4` | [journey/using-bitcoin §2](journey/using-bitcoin.md) — 22-second walkthrough of an on-chain transaction lifecycle. Alice's wallet (To / Amount / Fee / SEND), tx flies into a fee-sorted mempool and slots in at 20 sat/vB, top 4 selected into a block, block sealed and snapped to chain, Bob's wallet receives +0.1 BTC with confirmation counter ticking to Final. Closing pillars: Public / Final / Yours. The on-chain counterweight to the lightning-mesh hook at the top of the chapter. OG card derived from the t=14s frame at `/diagrams/og/bitcoin-lifecycle.png`. |
