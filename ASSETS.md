...
| **Threshold-of-keys.** | `multisig.mp4` | [glossary/multisig](glossary/multisig.md) — top-of-entry hook, 35-second walkthrough of 2-of-3 multisig: setup (three different makers), normal spend (two signatures), loss scenario (one key gone, two remain), theft scenario (thief halts at one signature), closing pillars (Multisig / Threshold-of-keys / Vendor-diverse). First animated glossary entry. |
